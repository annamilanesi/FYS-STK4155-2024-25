...
class LinearRegression:
    ...
    def mse_polydegree(self, x, y, z, maxdegree, method):
        # Initialize arrays for MSE values (from degree 1 to maxdegree)
        ...
        # Loop over polynomial degrees from 1 to maxdegree
        for degree in range(1, maxdegree + 1):
            # Create design matrices for train and test sets
            X_train = design_matrix(x_train, y_train, degree)
            X_test = design_matrix(x_test, y_test, degree)
            
            self.beta_initial = None  # Reset initial beta if necessary
            ...
            mse_train[degree - 1] = mse(z_train, z_tilde)
            mse_test[degree - 1] = mse(z_test, z_pred)

        # Plot the MSE trend for each polynomial degree
        plt.figure(figsize=(10, 6))
        plt.plot(range(1, maxdegree + 1), mse_train, label='MSE Train', marker='o')
        plt.plot(range(1, maxdegree + 1), mse_test, label='MSE Test', marker='o')
    ...
    def mse_epochs_vs_batchsize(self, X_train, y_train, X_test, y_test, method, n_epochs_list, minibatch_sizes):
        ...
        for i, n_epochs in enumerate(n_epochs_list):
            for j, minibatch_size in enumerate(minibatch_sizes):
                ...
                y_tilde, y_pred = self.predict_all(X_train, X_test, beta)

                # Calculate MSE for training and testing sets
                mse_train[i, j] = mse(y_train, y_tilde)
                mse_test[i, j] = mse(y_test, y_pred)

        # Create heatmaps for visualization
        self.plot_heatmaps(mse_train, mse_test, minibatch_sizes, n_epochs_list, method, xlabel='Batch size', ylabel='Epochs')
    ...
    def mse_learningrate_vs_lmd(self, X_train, y_train, X_test, y_test, method, learning_rate_values, lmd_values):
        ...
        # Create heatmaps for visualization
        self.plot_heatmaps(mse_train, mse_test, lmd_values, learning_rate_values, method, xlabel='Lambda', ylabel='Learning Rate')
    ...
    def mse_batchsize_vs_lambda(self, X_train, y_train, X_test, y_test, method, batch_sizes, lambdas):

        # Initialize matrices to store MSE values
        mse_train = np.zeros((len(batch_sizes), len(lambdas)))
        mse_test = np.zeros((len(batch_sizes), len(lambdas)))

        # Loop over batch sizes and lambda values
        for i, batch_size in enumerate(batch_sizes):
            for j, lmbd in enumerate(lambdas):
                # Update batch size, lambda for each run
                self.batch_size = batch_size
                self.lmd = lmbd
        ...
        self.plot_heatmaps(mse_train, mse_test, lambdas, batch_sizes, method, xlabel='Lambda', ylabel='Batch size')
