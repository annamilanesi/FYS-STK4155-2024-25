...
def plot_heatmaps_acc(acc_train, acc_test, x_labels, y_labels, method, xlabel='X-axis', ylabel='Y-axis'):
    # Create heatmap for visualization
    plt.figure(figsize=(14, 6))
    ...
    plt.subplot(1, 2, 1)
    sns.heatmap(acc_train, annot=True, fmt=".4f", xticklabels=x_labels, yticklabels=y_labels, cmap="viridis")
    ...
    plt.subplot(1, 2, 2)
    sns.heatmap(acc_test, annot=True, fmt=".4f", xticklabels=x_labels, yticklabels=y_labels, cmap="viridis")
...
def accuracy_epochs_vs_batchsize(X_train, X_test, z_train, z_test, n_epochs_list, batch_sizes, neural_network_model, learning_rate=0.01):
    # Initialize matrices for accuracy values
    acc_train = np.zeros((len(n_epochs_list), len(batch_sizes)))
    acc_test = np.zeros((len(n_epochs_list), len(batch_sizes)))
    ...
    for i, epoch in enumerate(n_epochs_list):
        for j, batch_size in enumerate(batch_sizes):
            ...
            acc_train[i, j] = accuracy(z_tilde, z_train)
            acc_test[i, j] = accuracy(z_pred, z_test)

            neural_network_model.reset_weights()

    plot_heatmaps_acc(acc_train, acc_test, batch_sizes, n_epochs_list, method="Neural Network", xlabel='Batch Size', ylabel='Epochs')

    return acc_train, acc_test

def accuracy_learning_rate_vs_lambda(X_train, X_test, z_train, z_test, learning_rates, lambdas, n_epochs, batch_size, neural_network_model):
    # Initialize matrices for accuracy values
    acc_train = np.zeros((len(learning_rates), len(lambdas)))
    acc_test = np.zeros((len(learning_rates), len(lambdas)))
    ...
    plot_heatmaps_acc(acc_train, acc_test, lambdas, learning_rates, method="Neural Network", xlabel='Lambda', ylabel='Learning Rate')

    return acc_train, acc_test

def accuracy_batchsize_vs_learning_rate(X_train, X_test, z_train, z_test, batch_sizes, learning_rates, n_epochs, neural_network_model):
    # Initialize matrices for accuracy values
    acc_train = np.zeros((len(batch_sizes), len(learning_rates)))
    acc_test = np.zeros((len(batch_sizes), len(learning_rates)))
    ...
    plot_heatmaps_acc(acc_train, acc_test, batch_sizes, learning_rates, method="Neural Network", xlabel='Batch Size', ylabel='Learning Rate')

    return acc_train, acc_test


# Function that fixes a number of hidden layers and varies the number of nodes
def accuracy_n_nodes(X_train, X_test, z_train, z_test, hidden_layers, n_nodes_list, 
                    n_epochs=100, batch_size=10, learning_rate=0.01, output_size=1, 
                    hidden_activation_func=sigmoid, hidden_activation_der=sigmoid_der, 
                    output_activation_func=sigmoid, output_activation_der=sigmoid_der,
                    cost_fun=mse, cost_der=mse_der):
    ...
    for n_nodes in n_nodes_list:
        ...
        # Calculate accuracy for training and testing data
        accuracy_train.append(accuracy(z_train_pred, z_train))
        accuracy_test.append(accuracy(z_test_pred, z_test))
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(n_nodes_list, accuracy_train, label='Train Accuracy', marker='o')
    plt.plot(n_nodes_list, accuracy_test, label='Test Accuracy', marker='x')
...
# Function that fixes a number of nodes per layer and varies the number of hidden layers
def accuracy_n_hidden_layers(X_train, X_test, z_train, z_test, n_nodes, n_hidden_layers_list,
                            n_epochs=100, batch_size=10, learning_rate=0.01, output_size=1, 
                            hidden_activation_func=sigmoid, hidden_activation_der=sigmoid_der, 
                            output_activation_func=sigmoid, output_activation_der=sigmoid_der, 
                            cost_fun=mse, cost_der=mse_der):
    ...
    for n_hidden_layers in n_hidden_layers_list:
        ...
        # Train the neural network
        train_layers_SGD(X_train, z_train, neural_network_model, M=batch_size, n_epochs=n_epochs, learning_rate=learning_rate)

        # Predict on training and testing data
        z_train_pred = neural_network_model.predict(X_train)
        z_test_pred = neural_network_model.predict(X_test)
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(n_hidden_layers_list, accuracy_train, label='Training Accuracy', marker='o')
    plt.plot(n_hidden_layers_list, accuracy_test, label='Testing Accuracy', marker='o')
